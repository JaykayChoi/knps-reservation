...
def list_active_settings():
    return jsonify(_deps().monitors.list(active_only=True))


@blueprint.get('/api/settings/all')
def list_all_settings():
    return jsonify(_deps().monitors.list())


@blueprint.get('/api/settings/<int:monitor_id>')
def get_setting(monitor_id):
    value = _deps().monitors.get(monitor_id)
    return (jsonify(value), 200) if value else (jsonify({'error': 'Monitor not found'}), 404)
...
def create_setting():
    try:
        ...
        return jsonify({'success': True, 'setting': created}), 201
...
def update_setting(monitor_id):
    ...
    try:
        ...
        return jsonify({'success': True, 'setting': updated})
    except ValueError as exc:
        return jsonify({'error': str(exc)}), 400
