...
class TestDeleteHistoryBySetting:
    """Tests for delete_history_by_setting() function."""
    
    def test_delete_history_by_setting_success(self, mocker):
        """Test successful deletion of history for a specific setting."""
        mock_client = Mock()
        mock_table = Mock()
        mock_delete = Mock()
        mock_execute = Mock()
        ...
        mocker.patch('db.get_supabase', return_value=mock_client)
        db.delete_history_by_setting(1)
        mock_table.delete.assert_called_once()
        mock_delete.eq.assert_called_once_with("setting_id", 1)
        mock_delete.execute.assert_called_once()

class TestDeleteAllHistory:
    """Tests for delete_all_history() function."""
    
    def test_delete_all_history_success(self, mocker):
        """Test successful deletion of all notification history."""
        mock_client = Mock()
        mock_table = Mock()
        mock_delete = Mock()
        mock_execute = Mock()
        ...
        mock_client.table.return_value = mock_table
        mock_table.delete.return_value = mock_delete
        mock_delete.neq.return_value = mock_delete
        mock_delete.execute.return_value = mock_execute
        
        mocker.patch('db.get_supabase', return_value=mock_client)
        db.delete_all_history()
        mock_table.delete.assert_called_once()
        mock_delete.neq.assert_called_once_with("id", -1)
        mock_delete.execute.assert_called_once()
#BZ|
