...
class ModuParkingError(RuntimeError):
    pass
...
def fetch_monthly_passes(lots):
    """지정한 주차장들의 월정기권 상태를 조회한다.

    Args:
        lots: catalog에서 읽은 ``{seq, name, geohash}`` 목록.
    ...
    for lot in lots:
        ...
        pin_result = pins_cache[geohash]
        pin = pin_result.get(lot["seq"]) if pin_result is not None else None
        if pin:
            ...
            if tickets is None:
                raise ModuParkingError(f"Parking provider failed for lot {lot['seq']}")
